...
(function (window, document, undefined) {
    ...
    App.TimeScheduleModule.enableTimeIndicator(mainTimeScheduleContainer);
    ...
    mainTimeScheduleContainer.addEventListener("timeScheduleProgress", function () {
        let progressInPercent = Math.round(App.TimeScheduleModule.getProgress(mainTimeScheduleContainer) * 100);
    ...
    let fightPlaceSelect = App.MaterialInputsModule.getInputObject(fightPlaceSelectElement);
    ...
    fightPlaceSelectElement.addEventListener("change", function () {
        ...
        App.TimeScheduleModule.updateProgress(detailViewPageTimeScheduleElement);
        App.TimeScheduleModule.enableTimeIndicator(detailViewPageTimeScheduleElement);
    ...
    let detailViewCloseBtn = document.querySelector(".category-detail-view .primary-button.close");
     ...
     */
    App.TooltipModule.createTooltip("a.previous-category", "Zeige die vorherige Kategorie dieses Pools");
    App.TooltipModule.createTooltip("a.next-category", "Zeige die nächste Kategorie dieses Pools");

    /**
     * This block initializes variables to identify the elements with dynamic content
     */
    let categoryStatuus = App.GeneralModule.generalVariables.categoryStatuus;
    ...
    categoryStatusControlBtn.addEventListener("click", function () {
        if (categoryStatusControlBtn.classList.contains("start-category")) {
            ...
            App.SecondaryButtonModule.disableSecondaryButton(categoryStatusControlBtn);
    ...
    function changeCategoryStatusControlBtn(removeClasses, addClasses, iconName, btnText) {
        ...
        let icon = App.GeneralModule.generateElement("i", ["material-icons"], iconName);
    ...
    function enterCategoryDetailedView(categoryName) {
        ...
        function initializeCategoryActionButtons() {
            if (categoryObject.status === categoryStatuus.DONE || categoryObject.status === categoryStatuus.TERMINATED) {
                App.PrimaryButtonModule.enablePrimaryButton(categoryPositioningBtn);
            }
        }
        ...
        function initializeCategoryStatusControlButton() {
            ...
            App.SecondaryButtonModule.enableSecondaryButton(categoryStatusControlBtn);
